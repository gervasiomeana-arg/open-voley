...
import { Player, MatchData, ScoutCodeAction } from '../types';
...
export const Player360Modal: React.FC<Player360ModalProps> = ({
  ...
  // Derive player actions from match if available
  const playerActions = (match?.actions || []).filter(
    (a) => a.playerNum === player.number && a.team === player.team
  );

  const attacks = playerActions.filter((a) => a.skill === 'A');
  const attackPoints = attacks.filter((a) => a.evaluation === '#').length;
  const attackErrors = attacks.filter((a) => a.evaluation === '=').length;
  const attackBlocked = attacks.filter((a) => a.evaluation === '/').length;
  const attackEff = attacks.length > 0 
    ? Math.round(((attackPoints - attackErrors - attackBlocked) / attacks.length) * 100)
    : 44;

  const receptions = playerActions.filter((a) => a.skill === 'R');
  const recPos = receptions.filter((a) => a.evaluation === '#' || a.evaluation === '+').length;
  const recPosPct = receptions.length > 0 ? Math.round((recPos / receptions.length) * 100) : 62;

  const serves = playerActions.filter((a) => a.skill === 'S');
  const aces = serves.filter((a) => a.evaluation === '#').length;
  const serveErrors = serves.filter((a) => a.evaluation === '=').length;

  const blocks = playerActions.filter((a) => a.skill === 'B');
  const blockPoints = blocks.filter((a) => a.evaluation === '#').length;

  const digs = playerActions.filter((a) => a.skill === 'D').length;

  const totalPoints = attackPoints + blockPoints + aces;
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  ...
                  <div className="text-[10px] text-slate-400 mt-1">{attackPoints} atq • {blockPoints} blq • {aces} ace</div>
                </div>

                <div className="p-3.5 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Eficiencia Ataque</div>
                  <div className="text-2xl font-black text-white font-mono mt-0.5">{attackEff}%</div>
                  <div className="text-[10px] text-emerald-400 mt-1">{attackPoints} puntos / {attacks.length} intentos</div>
                </div>

                <div className="p-3.5 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Recepción Positiva</div>
                  <div className="text-2xl font-black text-cyan-400 font-mono mt-0.5">{recPosPct}%</div>
                  <div className="text-[10px] text-slate-400 mt-1">{recPos} positivas / {receptions.length} totales</div>
                </div>

                <div className="p-3.5 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Defensas / Digs</div>
                  <div className="text-2xl font-black text-emerald-400 font-mono mt-0.5">{digs}</div>
                ...
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-[11px] mb-1">
                      ...
                      <span className="font-mono font-bold text-amber-400">{attackEff}%</span>
                    </div>
                    <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-400 rounded-full" style={{ width: `${Math.min(100, Math.max(10, attackEff))}%` }} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-[11px] mb-1">
                      ...
                      <span className="font-mono font-bold text-cyan-400">{recPosPct}%</span>
                    </div>
                    <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-cyan-400 rounded-full" style={{ width: `${recPosPct}%` }} />
                    </div>
                  </div>
          ...
          {activeTab === 'desarrollo' && (
            <div className="space-y-4">
              <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/60 space-y-3">
                <h4 className="font-black text-white text-xs flex items-center justify-between">
                  <span>Evolución en los últimos 4 partidos</span>
                ...
                <div className="grid grid-cols-4 gap-2 text-center pt-2">
                  {[
                    { match: 'vs San Lorenzo', eff: '38%', rec: '52%', score: 'G 3-1' },
                    { match: 'vs River Plate', eff: '42%', rec: '56%', score: 'G 3-0' },
                    { match: 'vs Boca Juniors', eff: '46%', rec: '58%', score: 'P 2-3' },
                    { match: 'vs Gimnasia LP', eff: '54%', rec: '65%', score: 'G 3-1' },
                  ].map((item, idx) => (
                    <div key={idx} className="p-2.5 bg-slate-900/80 rounded-xl border border-slate-800">
                      <div className="text-[10px] text-slate-400 truncate">{item.match}</div>
                      <div className="text-sm font-black text-white font-mono mt-1">{item.eff}</div>
                      <div className="text-[9px] text-cyan-400 font-mono">Rec: {item.rec}</div>
                      <span className="text-[9px] font-bold text-amber-400">{item.score}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800 space-y-2">
                <div className="text-[10px] font-bold text-slate-400 uppercase">Objetivos de Desarrollo Individual</div>
                <ul className="space-y-2 text-slate-300">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                    <span>Incrementar el ataque de primer tiempo por zona 3 frente a bloqueo doble.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                    <span>Mantener la plataforma de antebrazos fija en la recepción de saque flotado hacia zona 5.</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Saltos Estimados</div>
                  <div className="text-xl font-black text-white font-mono mt-0.5">64 saltos</div>
                  <div className="text-[10px] text-slate-400 mt-0.5">42 ataques + 22 bloqueos</div>
                </div>

                <div className="p-3 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Sets Jugados</div>
                  <div className="text-xl font-black text-amber-400 font-mono mt-0.5">4 sets</div>
                  <div className="text-[10px] text-slate-400 mt-0.5">100% de minutos en cancha</div>
                </div>

                <div className="p-3 bg-slate-800/60 rounded-2xl border border-slate-700/60">
                  <div className="text-[10px] text-slate-400 font-bold uppercase">Estado Físico</div>
                  <div className="text-xl font-black text-emerald-400 font-mono mt-0.5">Óptimo</div>
                  <div className="text-[10px] text-slate-400 mt-0.5">RPE post-partido: 7/10</div>
                </div>
              </div>

              {/* Medical and Nutrition Notes */}
              <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800 space-y-3">
                <div>
                  <div className="text-[10px] font-bold text-amber-400 uppercase tracking-wider mb-1">
                    Ficha Kinesiológica y Lesiones
                  </div>
                  <p className="text-slate-300">
                    {player.kinesiologyNotes || 'Sin observaciones médicas registradas. Mantenimiento preventivo habitual.'}
                  </p>
                </div>

                <div className="pt-2 border-t border-slate-800">
                  <div className="text-[10px] font-bold text-cyan-400 uppercase tracking-wider mb-1">
                    Pautas de Nutrición y Recuperación
                  </div>
                  <p className="text-slate-300">
                    {player.nutritionNotes || 'Plan de hidratación estándar pre y post competición.'}
                  </p>
                </div>
              </div>
            </div>
