...
function __OriginkitBase_ZoomTextTunnel(props: ZoomTextTunnelProps) {
  const {
    texts = ["scouting", "analisis", "estadisticas", "entrenador"],
    font = {
  ...
  const safeTexts = useMemo(() => {
    const cleaned = Array.isArray(texts)
      ...
      : [];
    return cleaned.length > 0 ? cleaned : ["OPEN VOLEY"];
  }, [texts]);
...
export default function ZoomTextTunnel(props: ZoomTextTunnelProps) {
  return <__OriginkitBase_ZoomTextTunnel {...(__originkitPresetProps as any)} {...props} />;
}
